...
| `allow_moving_cursor_after_close_or_minimise` | `true`        | Whether to move the cursor automatically to the closest window after closing or minimising a window. If set to `true`, the cursor will be moved to the closest visible window after you use a Randolf hotkey to close or minimise a window. Randolf does not use Windows API callbacks (yet) which can, for example, cause the cursor to move when the window to be closed did not close immediately but opened a separate confirmation pop-up before executing the command.                                        |
