...
impl ConfigurationProvider {
  ...
  pub fn set_bool(&mut self, name: &str, value: bool) {
    match name {
      ...
      ENABLE_FEATURES_USING_MOUSE => {
        self.config.general.enable_features_using_mouse = value;
        if let Err(err) = self.save_config() {
          error!("Failed to save configuration: {}", err);
        }
      }
  ...
  pub fn set_i32(&mut self, name: &str, value: i32) {
    match name {
      ...
      DELAY_IN_MS_BEFORE_DRAGGING_IS_ALLOWED => {
        self.config.general.delay_in_ms_before_dragging_is_allowed = value;
        if let Err(err) = self.save_config() {
          error!("Failed to save configuration: {}", err);
        }
      }
  ...
  fn save_config(&self) -> Result<(), Box<dyn Error>> {
    self.file_manager.save(&self.config)
  }
}
