...
impl TrayMenuManager {
  ...
  fn menu_builder(&self, version: &str, config: MutexGuard<ConfigurationProvider>) -> MenuBuilder<Event> {
    ...
    MenuBuilder::new()
      ...
      .checkable("Enable logging", config.get_bool(FILE_LOGGING_ENABLED), Event::ToggleLogging)
  ...
  fn initialise(&self, rx: Receiver<Event>) {
    ...
    thread::spawn(move || {
      rx.iter().for_each(|m| match m {
        ...
        Event::ToggleLogging => {
          let is_enabled = unlocked_config_provider(&config_provider).get_bool(FILE_LOGGING_ENABLED);
          ...
          unlocked_config_provider(&config_provider).set_bool(FILE_LOGGING_ENABLED, !is_enabled);
