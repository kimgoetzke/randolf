...
fn secondary_active_ws_id() -> &'static TransientWorkspaceId {
  SECONDARY_ACTIVE_WORKSPACE.get_or_init(|| TransientWorkspaceId::new(secondary_monitor().id, secondary_monitor().handle, 1))
}
...
impl WorkspaceManager<MockWindowsApi> {
  ...
  pub fn new_test(is_test_window_in_foreground: bool, path: PathBuf) -> Self {
    ...
    WorkspaceManager {
      workspaces: HashMap::from([
        ...
        (
          ...
          Workspace::new_inactive(primary_inactive_workspace_id.into(), primary_monitor, window_margin),
        ),
        (
          secondary_active_workspace_id.into(),
          Workspace::new_active(secondary_active_workspace_id.into(), secondary_monitor, window_margin),
        ),
        (
          secondary_inactive_workspace_id.into(),
          Workspace::new_inactive(secondary_inactive_workspace_id.into(), secondary_monitor, window_margin),
        ),
      ]),
...
#[test]
fn restore_hidden_windows_unhides_all_matching_windows() {
  ...
  let w_1 = Window::new_test(2, Rect::new(0, 0, 100, 100));
  let w_2 = Window::new_test(3, Rect::new(100, 100, 200, 200));
  MockWindowsApi::add_or_update_window(w_1.handle, w_1.title.clone(), w_1.rect.into(), false, true, false);
  MockWindowsApi::add_or_update_window(w_2.handle, w_2.title.clone(), w_2.rect.into(), false, true, false);
  let workspace_file = r#"
      [[workspaces."P_DISPLAY|1|true"]]
      hwnd = 2
      [[workspaces."P_DISPLAY|1|true"]]
      hwnd = 3
      "#;
  fs::write(&path, workspace_file).expect("Failed to write config file");

  workspace_manager.restore_hidden_windows_from_file();

  assert_eq!(workspace_manager.windows_api.get_all_visible_windows().len(), 3);
  assert!(workspace_manager.windows_api.get_all_visible_windows().contains(&w_1));
  assert!(workspace_manager.windows_api.get_all_visible_windows().contains(&w_2));
  assert_ne!(
    workspace_manager.windows_api.get_foreground_window(),
    Some(WindowHandle::from(1)),
    "The test window should not be the foreground window (because it'll be replaced by one from the workspace file)"
  );
  assert!(workspace_manager.workspace_file.workspaces.is_empty());
}

#[test]
fn restore_hidden_windows_does_nothing_if_no_matching_windows() {
  let directory = create_temp_directory();
  let path = directory.path().join(WORKSPACES_FILE_NAME);
  let mut workspace_manager = WorkspaceManager::new_test(false, path.clone());
  let workspace_file = r#"
      [[workspaces."P_DISPLAY|1|true"]]
      hwnd = 2
      [[workspaces."P_DISPLAY|2|true"]]
      hwnd = 42
      "#;
  fs::write(&path, workspace_file).expect("Failed to write config file");
  ...
  assert_eq!(
    workspace_manager.windows_api.get_all_visible_windows()[0].handle,
    WindowHandle::from(1),
    "The test window should be the only visible window as 2 and 42 are no longer present"
  );
  assert!(workspace_manager.workspace_file.workspaces.is_empty());
}
...
fn get_ordered_workspace_ids_left_to_right() {
  ...
  let mut workspace_manager = WorkspaceManager::from_workspaces(&[&left_workspace, &center_workspace, &right_workspace], 0);
...
fn get_ordered_workspace_ids_top_to_bottom() {
  ...
  let mut workspace_manager = WorkspaceManager::from_workspaces(&[&top_workspace, &center_workspace, &bottom_workspace], 0);
